...
//     2 	EMFK.0(KEK)* 				0, 16, 32 			0 - 9, A - F //KEK Encrypted under Variant 0 of MFK
...
//     1 	EMFK.V(Working Key) 		16, 32 				0 - 9, A - F //working key encrypted under VariantNN of MFK
//     2 	EKEK.V(Working Key) 		0, 16, 32 			0 - 9, A - F //working key encrypted under VariantNN of KEK
//     3 	Working Key Check Digits 	4 or 6 				0 - 9, A - F //New working Key's Key Check Value of 4 or 6 Chars
//--------------------------------------------------------------------------

public class GenerateWorkingKeyAnyType {
	...
	public static String Go(String sCommand) throws InvalidKeyException,
		...
		if (sCommand.matches("<10#\\d##\\w#>")) { // return a key not wrapped by KEK
			...
			byte[] b_working_key_VNN = CryptoUtils.xor(b_working_key, DatatypeConverter.parseHexBinary(s_Variant_Hex + s_Variant_Hex));
			...
			return "<20#" + new String(Hex.encode(b_working_key_VNN)).toUpperCase() +"#" + s_KCV + "#>";
		}

		
		// Generate a 2key-3DES (double-length) PIN Encryption Key.** Variant: 1
		// Example: application sends this request: <10#1#9007B8751BB7AB4EE355AF51A716113F#D#>
		...
		if (sCommand.matches(".*<10#\\d#.*")) {
			...
			byte[] bKEKX3 = CryptoUtils.xor(bKEKX2, DatatypeConverter.parseHexBinary(s_Variant_Hex + s_Variant_Hex));// load VariantNN of the doubled up KEK
			...
			return "<20#" + new String(Hex.encode(eMFK_V_Working_Key)).toUpperCase() + "#" + new String(Hex.encode(eKEK_V_Working_Key)).toUpperCase() + "#" + s_KCV + "#>";
