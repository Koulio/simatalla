...
//2 		EMFK.V(Working Key)* 		16, 32 				0 - 9, A - F
...
//1 		EPending MFK.V(Working Key) 	16, 32 				0 - 9, A - F
//2 		Working Key Check Digits 		4 or 6 				0 - 9, A - F
//--------------------------------------------------------------------------

public class TranslateWorkingKeyforLocalStorageUnderCurrentMFKToPendingMFK {
...
if (sCommand.matches("<18#\\d##\\w#>")) { // return a key not wrapped by KEK
	...
	return "<28#" + new String(Hex.encode(b_working_key_VNN)).toUpperCase() +"#" + s_KCV + "#>";
}


// Generate a 2key-3DES (double-length) PIN Encryption Key.** Variant: 1
// Example: application sends this request: <18#1#9007B8751BB7AB4EE355AF51A716113F#D#>
...
if (sCommand.matches(".*<18#\\d#.*")) {
	...
	return "<28#" + new String(Hex.encode(eMFK_V_Working_Key)).toUpperCase() + "#" + new String(Hex.encode(eKEK_V_Working_Key)).toUpperCase() + "#" + s_KCV + "#>";
